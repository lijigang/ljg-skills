...
卡身保持浅色；地形图自带背景（动森奶油绿或 cyber 暗黑），是卡上唯一的重色块——这层深浅对比是特意留的。

## 强调色 {{ACCENT}}

用在 kicker、标签、英文行、thesis 关键词高亮、大问题序号、base rate 数字、署名印。动森 mold 挑暖色或草绿（`#5a9e1e` / `#c47a1a` / `#725d42`）配奶油地形；cyber mold 挑霓虹色（靛蓝/青）。和地形图自带配色互不干扰。

## 字体（卡身文字）

thesis / 大问题 / 行业名 → Noto Serif SC；标签 / EN / base rate 数字 → JetBrains Mono。地形图里的地名是生成图自带的（动森木牌字、cyber 终端字），不归 CSS 管。
