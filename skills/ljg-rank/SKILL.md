...
写入 `~/Downloads/{领域}--rank.html`，报告路径，完成。
...
1. **基础外壳**：使用 `references/html-patterns.md` 中的基础 HTML 外壳，标题为"{领域}的秩"
2. **上半区——生成器卡片**：使用 Pattern 7 拖拽排序卡片列表，每个生成器一张卡片。卡片内容：生成器名 + 一句话描述。用户可拖拽重排优先级。卡片颜色深浅表示该生成器的"根性"（越深越根）
3. **下半区——关系骨架图**：使用 Pattern 6 内联 SVG，画出生成器之间的关系结构图。根据 root rank 的拓扑形态选择对应画法（钻井剖面/并排/反馈环/阶段链/树形/网状/光谱/振荡/多切片）。节点可点击高亮
4. **叠加坐标系**（如有）：在骨架图下方再画一个 SVG 坐标系图
5. **导出栏**：使用 Pattern 5 的固定底部导出栏，提供"复制为 Org-mode"和"复制为 Markdown"按钮。导出内容包含排序后的生成器列表 + ASCII 关系图

### 文件输出

写入 `~/Downloads/{领域}--rank.html`。
